...
module ScoringHelper
  ...
  def score_draw(matchday, teams)
    Score.where(matchday_id: matchday, team_id: teams).update(points: 1)
  end
  ...
  def score_non_draw(matchday, home_team, home_goals, away_team, away_goals)
    ...
    Score.where(matchday_id: matchday, team_id: winner).update(points: scores.max)
    Score.where(matchday_id: matchday, team_id: loser).update(points: scores.min)
  end
  ...
  def top_six?(team)
    %w[LIV MCI MUN CHE LEI TOT].include?(team)
  end

  def newly_promoted?(team)
    %w[LEE WBA FUL].include?(team)
  end
